...
export function CourseModal({ slot, onClose }: { slot: Slot; onClose: () => void }) {
  ...
  // Track expanded weeks; current week auto-expanded
                      ...
                      return (
                        ...
                        >
                          <div className="flex-1 min-w-0">
                            ...
                            {cd.date && <span className="text-[11px] text-muted-fg ml-2 tabular-nums">{formatHrDate(cd.date)}</span>}
                            {!cd.date && <span className="text-[11px] text-muted-fg ml-2">T{cd.week}</span>}
                          </div>
                          {days !== null && days >= 0 && (
                            <span
